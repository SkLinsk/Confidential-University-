...
void Delete_Linklist_index( Linklist head, int index )
{
    if( head->next == NULL )
    {
    ...
    int j = 0;
    Linklist current, advance, previous;
    current = head;
    ...
    else if ( ( current->next == NULL ) && ( j == index - 1 ) )
    {
        free( current );
        previous->next = NULL;
    }
...
void Delete_Linklist_data( Linklist head, elemtype data )
{
    ...
    int flag = 0;
    Linklist current, advance, previous, temp;
    current = head;
...
void Reverse_Linklist( Linklist &head )
{
    ...
    Linklist advance, temp, previous = NULL;
    while ( current->next != NULL )
    {
        advance = current->next;
...
int main()
{
    ...
    Init_Linklist( head4 );
    Init_Linklist( head5 );
    int arr4[ 3 ] = { 1, 2, 3 }, arr5[ 4 ] = { 1, 2, 3, 4 };
